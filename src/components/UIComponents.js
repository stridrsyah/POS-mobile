 ...
 * Reusable, polished UI components untuk POS Mobile v2
 * Design: Deep Navy Dark + Clean White Light
 */
...
export const Button = ({
  ...
  icon, iconRight, loading, disabled, style, labelStyle,
  fullWidth = true,
}) => {
  const { colors } = useTheme();
  ...
  const variantStyle = {
    primary: {
      bg: disabled ? colors.textDark : colors.primary,
      text: '#FFFFFF',
      border: 'transparent',
    },
    secondary: {
    ...
    danger: {
      bg: disabled ? colors.textDark : colors.danger,
      text: '#FFFFFF',
      border: 'transparent',
    },
    ghost: {
    ...
    success: {
      bg: disabled ? colors.textDark : colors.success,
      text: '#FFFFFF',
      border: 'transparent',
    },
  }[variant];
  ...
  return (
    ...
    >
      {loading ? (
        ...
        <>
          {icon && !iconRight && (
            <Ionicons name={icon} size={s.fontSize + 1} color={variantStyle.text} style={{ marginRight: 7 }} />
          )}
          <Text style={[btnS.label, { color: variantStyle.text, fontSize: s.fontSize }, labelStyle]}>
            {label}
          </Text>
          {icon && iconRight && (
            <Ionicons name={icon} size={s.fontSize + 1} color={variantStyle.text} style={{ marginLeft: 7 }} />
          )}
        </>
      )}
    </TouchableOpacity>
  ...
  return (
    <View style={[obS.wrap, { backgroundColor: colors.warning + '18', borderColor: colors.warning + '44' }]}>
      ...
      <Text style={[obS.text, { color: colors.warning }]}>
        Mode Offline
        {pendingCount > 0 ? ` • ${pendingCount} transaksi menunggu sync` : ''}
      </Text>
    </View>
  );
};
...
export const Divider = ({ my = SPACING.md }) => {
  const { colors } = useTheme();
  return <View style={{ height: 1, backgroundColor: colors.divider, marginVertical: my }} />;
};
...
export const Skeleton = ({ width, height, borderRadius, style }) => {
  ...
  return (
    <Animated.View
      style={[
        { width, height: height || 16, borderRadius: borderRadius || RADIUS.sm, backgroundColor: bg },
        style,
      ]}
    />
  );
